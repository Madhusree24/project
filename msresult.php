...
if($ans->num_rows>=1)
{
    ...
    echo "<div style=margin-top:40px;margin-left:600px;width:250px;height:270px;float:left;>";
	echo "<caption><h2> MSOffice_RESULT </h2></caption>";
	echo "<table border=2>";
	echo "<tr><th>UserID</th><th>Username</th><th>Maxmarks</th><th>Marks</th></tr>";
	while($row=$ans->fetch_assoc())
	{
		echo "<tr><td>".$row['login']."</td><td>".$row['user']."</td><td>".$row['maxmarks']."</td>
		<td>".$row['marks']."</td></tr>";
	}
	echo "</table><br>";
	echo "<center><h3> <a href=scores.php><input type=button name=back value=Back></button> </a></h3></center></div>";
